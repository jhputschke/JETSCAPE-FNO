...
RegisterJetScapeModule<PGunFno> PGunFno::reg("PGunFno");

Pythia8::Pythia PGunFno::InternalHelperPythia("IntentionallyEmpty", false);

PGunFno::PGunFno() : HardProcess() {
  fixed_pT = 0;
  parID = 21;
  SetId("PGunFno");
  VERBOSE(8);
...
PGunFno::~PGunFno() { VERBOSE(8); }

void PGunFno::InitTask() {
  JSDEBUG << "Initialize PGun Brick (Test) ...";
  VERBOSE(8);

  std::string s = GetXMLElementText({"Hard", "PGunFno", "name"});
  JSDEBUG << s << " to be initialized ...";

  fixed_pT = GetXMLElementDouble({"Hard", "PGunFno", "pT"});
  JSDEBUG << s << " with fixed pT = " << fixed_pT;
  JSINFO << "Parton Gun with fixed pT = " << fixed_pT;

  parID = GetXMLElementDouble({"Hard", "PGunFno", "parID"});
