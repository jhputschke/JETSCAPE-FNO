Short C++ code to read through an input ROOT file with a TTree named `t` with a
branch of `vector<vector<vector<vector<float>,float>,float>,float>` named
`user_res` and writes a new TFile with a TTree and TBranch with the same names
as the input TFile, but with the data in the TBranch flattened into a single
fixed-sized array of floats.
